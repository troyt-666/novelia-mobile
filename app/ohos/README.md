...
通过 AppTest 安装的测试版可在手机上接收后续更新，无需连接电脑。
发布签名、上传和测试组配置见[内部测试分发](../../docs/harmonyos-apptest.md)。
2026-09-18 已通过命令行提交修复文库 R18 筛选的 `1.0.0（21）`，提交后为“预审中”。
第 19 版已显示“正在测试”；第 21 版实际更新安装仍待验证，发布状态详见上述分发记录。
...
- GitHub Release 提供未签名 HAP，供开发者使用自己的证书和已登记设备签名安装；
  本机绑定测试设备的签名包不公开分发。签名环境与源码构建步骤见上文。
- 鸿蒙暂时关闭 GitHub 应用内更新检查；AppTest 安装通过 AppTest 更新，
  本地 HAP 调试安装仍需手动更新。
