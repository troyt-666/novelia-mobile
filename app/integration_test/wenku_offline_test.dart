...
void main() {
  ...
  testWidgets('offline catalog opens new and legacy downloads in native WebView', (
    tester,
  ) async {
    final root = await Directory.systemTemp.createTemp('wenku-native-offline-');
    ...
    await store.save(
      ...
      wenkuOfflineEpub(longChapter: true),
      title: '下载的离线列车',
    );
    await File(
    ...
    for (final title in ['下载的离线列车', '旧版离线列车']) {
      ...
      expect(tester.takeException(), isNull);
      if (title == '下载的离线列车') {
        // Use the real EPUB engine to save a page, then reopen through the
        // offline list and check the restored page after layout is ready.
        for (var attempt = 0; attempt < 100; attempt++) {
          final ready = (await controller.runJavaScriptReturningResult(
            'readerProgress()',
          )).toString();
          if (ready.contains('"ready":true')) break;
          await tester.pump(const Duration(milliseconds: 50));
        }
        await controller.runJavaScript('readerSetFraction(0.6);');
        final download = (await store.listDownloads()).singleWhere(
          (item) => item.title == title,
        );
        WenkuReadingPosition? saved;
        for (var attempt = 0; attempt < 80; attempt++) {
          await tester.pump(const Duration(milliseconds: 50));
          saved = await store.readingPosition(download.fileName);
          if ((saved?.fraction ?? 0) > .4) break;
        }
        expect(saved?.fraction, greaterThan(.4));
        await tester.pump(const Duration(milliseconds: 500));
        debugPrint(
          'Offline EPUB before close: ${await controller.runJavaScriptReturningResult('readerProgress()')}',
        );
        await tester.tap(find.byType(BackButton));
        await tester.pumpAndSettle();
        final beforeClose = saved!.fraction;
        saved = await store.readingPosition(download.fileName);
        debugPrint('Offline EPUB saved: ${saved?.fraction}');
        expect(saved!.fraction, beforeClose);
        await tester.tap(find.text(title));
        await tester.pumpAndSettle();
        ...
        var restored = 0.0;
        for (var attempt = 0; attempt < 100; attempt++) {
          final value = await reopened.runJavaScriptReturningResult(
            'readerProgress()',
          );
          try {
            var parsed = value is String ? jsonDecode(value) : value;
            if (parsed is String) parsed = jsonDecode(parsed);
            if (parsed is Map && parsed['ready'] == true) {
              restored = (parsed['fraction'] as num).toDouble();
              if (restored > .4) break;
            }
          } on Object {
        ...
        debugPrint(
          'Offline EPUB reopened: ${await reopened.runJavaScriptReturningResult('readerProgress()')}',
        );
        expect(restored, closeTo(saved.fraction, .05));
      }
      await tester.tap(find.byType(BackButton));
      await tester.pumpAndSettle();
    }
