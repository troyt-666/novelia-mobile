...
void main() {
  ...
  test('shelf loads all referenced local novels without account filtering', () {
    ...
    final snapshot = LocalLibrarySnapshot.load(repository, knownNovels: known);
    expect(
      snapshot.continuedReads.map((row) => row.novel.id),
      containsAll([general.id, restricted.id]),
    );
    final coldStart = LocalLibrarySnapshot.load(
      repository,
      knownNovels: const [],
    );
    expect(
      coldStart.continuedReads.map((row) => row.novel.id),
      containsAll([general.id, restricted.id]),
    );
  });
}
