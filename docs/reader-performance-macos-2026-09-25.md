...
后续补充：用户确认至少上周末的版本（应为第 21 版）已经有同样的网文掉帧。
第 22 版的补图负载不能解释这个时间线。旧版本已有路径及本轮测试盲区见
[代码审查记录](reader-jank-code-audit-2026-09-25.md)。
后续已用真实文章及完整大目录重测，复现了补图之外的普通下载恢复阻塞，见
[真实文章测试](reader-real-content-macos-2026-09-25.md)。本文件保留生成数据实验。
...
- 没有后台下载任务时，纯文字和离线插图的连续正反向滚动没有复现持续掉帧。
  ...
  此结论仅适用于每章 6 段、fixture 正文供应器的小样本；未覆盖真实长章节、
  生产缓存读取和校验、普通下载恢复，且这两组未记录帧启动延迟。
